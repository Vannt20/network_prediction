...
from features.manifest import build_manifest, sha256_file
...
def prepare_feature_store(dataset_name, split_ratio=(0.7, 0.1, 0.2), k_lags=None, align_to_seq_len=True, data_dir=None, seed=42):
    """
    ...
    # I3 - khoá dữ liệu bằng manifest. Mọi artifact ghi ra đĩa kèm manifest này,
    # mọi artifact đọc vào bị đối chiếu với nó và RAISE nếu lệch. Đây là cơ chế
    # ngăn lỗi B2 (trộn hai phiên bản pipeline) tái diễn.
    if data_dir is None:
        data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data')
    csv_path = next((os.path.join(data_dir, f'{n}.csv')
                     for n in (ds_key, ds_key.upper(), ds_key.lower())
                     if os.path.exists(os.path.join(data_dir, f'{n}.csv'))), None)
    metadata['csv_sha256'] = sha256_file(csv_path) if csv_path else 'unknown'
    metadata['n_steps_after_hygiene'] = total_steps
    metadata['split_train'] = train_size
    metadata['split_val'] = val_size
    metadata['split_test'] = total_steps - train_size - val_size
    metadata['seq_len'] = seq_len
    # Số cửa sổ trượt của nhánh học sâu (create_sliding_windows bỏ seq_len bước đầu).
    metadata['n_windows_val'] = max(0, len(val_norm) - seq_len)
    metadata['n_windows_test'] = max(0, len(test_norm) - seq_len)
